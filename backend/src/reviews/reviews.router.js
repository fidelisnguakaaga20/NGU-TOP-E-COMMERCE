...
router.post("/post-review", async (req, res) => {
    try {
        ...
        const existingReview = await Reviews.findOne({ ProductId: productId, userId });
        ...
        const newReview = new Reviews({
            comment,
            rating,
            ProductId: productId,   // <-- capital P to match the schema
            userId,
            });
...
router.get("/total-reviews", async (req, res) => {
    try {
        ...
        console.error("Error getting total posting review", error);
