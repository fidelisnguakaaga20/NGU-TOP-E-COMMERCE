...
router.patch("/update-product/:id", verifyToken, verifyAdmin, async (req, res) => {
    try {
        ...
        onsole.error("Error Updating the product", error);
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        onsole.error("Error deleting the product", error);
...
router.get("/related/:id" , async (req, res) => {
    try {
        ...
        if(!product) {
            return res.status(404).send({ messagae: "Product not found" });
        }
        ...
        const relatedProducts = await Products.find({_id: {$ne: id},
        $or: [
    ...
    res.status(200).send(relatedProducts);
        ...
        onsole.error("Error fetching the related products", error);
